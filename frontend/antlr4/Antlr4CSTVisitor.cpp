...
std::any MiniCCSTVisitor::visitUnaryExp(MiniCParser::UnaryExpContext * ctx)
{
    ...
    if (ctx->primaryExp()) {
        ...
        return ast_node::New(ast_operator_type::AST_OP_NEG, expr, nullptr);
    } else {
        return nullptr;
    }
}
...
std::any MiniCCSTVisitor::visitPrimaryExp(MiniCParser::PrimaryExpContext * ctx)
{
    ...
    if (ctx->T_DIGIT()) {
        ...
        if (numStr.size() >= 2 && (numStr.substr(0, 2) == "0x" || numStr.substr(0, 2) == "0X")) {
            // 十六进制数
            val = (uint32_t)stoull(numStr, nullptr, 16);
        } else if (numStr.size() >= 1 && numStr[0] == '0' && numStr.size() > 1) {
            // 八进制数
            val = (uint32_t)stoull(numStr, nullptr, 8);
        } else {
            // 十进制数
            val = (uint32_t)stoull(numStr, nullptr, 10);
        }
